 ...
 */
// version: v20260620 (RK_portal統合に伴いGAS_URLを共通設定化、PWモード連携を追加)

// GAS URLは shared/ss-config.js の getGasUrl() から取得する（担当者ごとに自動切替）
const GAS_URL = (typeof getGasUrl === "function" ? getGasUrl("yoyaku") : null);
...
function startPatrol() {
  ...
  // 現在選択中のPWモード（mode1/mode2）をGAS経由でmain.pyまで連携する
  const pwMode = (typeof getCurrentPwMode === "function" ? getCurrentPwMode() : null) || "mode1";

  callGAS('triggerGitHubAction', { targetArea: select.value, pwMode })
    .then(res => {
      if (res === 'OK') { 
        btn.textContent = '巡回中...'; 
